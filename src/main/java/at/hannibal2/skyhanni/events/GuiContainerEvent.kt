...
abstract class GuiContainerEvent(open val gui: SkyHanniGuiContainer, open val container: AbstractContainerMenu) : SkyHanniEvent() {
    ...
    data class SlotClickEvent(
        ...
        fun makePickblock() {
            if (this.clickedButton == 2 && this.clickType == ContainerInput.MIDDLE) return
            slot?.index?.let { slotNumber ->
                InventoryUtils.clickSlot(slotNumber, container.containerId, mouseButton = 2, mode = ContainerInput.MIDDLE)
                cancel()
            }
        }
    }

    enum class ContainerInput(val id: Int) {
        NORMAL(0),
        SHIFT(1),
        HOTBAR(2),
        MIDDLE(3),
